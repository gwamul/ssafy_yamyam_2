...
## ◼ 추천 시스템 및 음식 검색 알고리즘 구현

### 1. 기술적 구현 배경

본 서비스는 사용자의 식단 선택을 보조하기 위해 단순 검색 기능을 넘어,
**영양 성분 기반 + 음식 유형(장르) 기반의 추천 알고리즘**을 적용했습니다.

기존의 단순 키워드 검색은 음식 간 우선순위를 제공하지 못하는 한계가 있었으며,
특히 “다이어트”, “벌크업” 등 목적에 따라 적합한 음식이 달라지는 문제를 해결하기 위해
**가중치 기반 점수화 시스템 + 카테고리 보정 로직**을 설계했습니다.

---

### 2. 핵심 구현 로직 (Source Code Logic)

#### A. 전체 음식 DB 기준 정규화 (Normalization)

모든 음식은 개별 비교가 아닌, 전체 DB를 기준으로 상대 평가됩니다.

1. 음식 DB 로드 후 `calculateDBStats()` 실행
2. 각 영양소(에너지, 단백질, 지방 등)의 최소/최대값 계산
3. `(값 - 최소) / (최대 - 최소)` 방식으로 정규화 수행

> > 이를 통해 음식 간 상대적인 건강도를 계산 가능

---

#### B. 목표 기반 가중치 시스템 (Diet Preset)

사용자의 식단 목적에 따라 각 영양소의 중요도를 다르게 반영합니다.
...
#### C. 음식 점수 계산 알고리즘 (Scoring)

각 음식은 다음 과정을 통해 점수화됩니다.

1. 영양소별 정규화 수행
2. 단백질을 제외한 항목은 “낮을수록 좋음” → 점수 반전
3. 가중치 적용 후 평균 계산
4. 최종 점수를 0~100 범위로 변환
...
#### D. 음식 카테고리 기반 보정 (Category Boosting)

단순 영양값만으로는 음식 “종류”를 반영할 수 없기 때문에
**식품명 키워드를 활용한 장르 보정 로직**을 추가했습니다.
...
### 3. 사용자 인터랙션 (UI 기반 추천 제어)

사용자는 UI에서 추천 모드를 직접 선택할 수 있으며,
선택된 모드는 전역 변수 `currentMode`를 통해 즉시 반영됩니다.

- 다이어트 / 벌크 / 키토 / 건강식 선택 가능
- 선택 시 검색 결과 자동 재정렬
...
- 단순 검색 → **목표 기반 추천 시스템으로 확장**
- 사용자 의도(다이어트 등)에 맞는 음식 우선 노출
- 음식 “영양 + 의미”를 동시에 반영한 하이브리드 추천 구현
- UX 향상 (직관적인 결과 제공)
