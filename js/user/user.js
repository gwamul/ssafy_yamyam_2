...
window.handleAddFollower = async function() {
    ...
    if (!targetId || targetId === myId) {
        alert(targetId === myId ? '본인은 팔로우할 수 없습니다.' : '아이디를 입력하세요.');
        return;
    }
...
window.handleAvatarUpload = function(event) {
    ...
    if (file.size > 1024 * 1024) { alert('1MB 이하 파일만 가능합니다.'); return; }
    ...
    reader.onload = async function(e) {
        ...
        try {
            ...
            if (res.ok) {
                document.getElementById('profileAvatarDisplay').innerHTML = `<img src="${base64Image}" class="avatar-img" alt="Profile">`;
...
function renderSubscribedChallenges() {
    ...
    if (mySubscribed.length === 0) {
        listContainer.innerHTML = '<div class="col-12 text-center py-5"><p class="text-muted">참여 중인 챌린지가 없습니다.</p></div>';
        return;
    }
    listContainer.innerHTML = mySubscribed.map(challenge => {
        const themeColor = challenge.difficulty === 'hard' ? '#ff3b30' : challenge.difficulty === 'medium' ? '#ff9500' : '#34c759';
