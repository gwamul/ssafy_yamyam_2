        ...
        try {
            ...
            if (req.url === '/api/user/register' && req.method === 'POST') {
                if (userDb.has(data.id)) {
                    ...
                    return res.end(JSON.stringify({ message: '이미 존재하는 아이디입니다.' }));
                }
                const salt = await bcrypt.genSalt(10);
                const hashedPassword = await bcrypt.hash(data.pw, salt);
                userDb.set(data.id, { pw: hashedPassword, profile: data.profile, diets: [], followers: [] });
            ...
            if (req.url === '/api/user/login' && req.method === 'POST') {
                ...
                if (!user || !(await bcrypt.compare(data.pw, user.pw))) {
                    ...
                    return res.end(JSON.stringify({ message: '아이디 또는 비밀번호가 틀립니다.' }));
                }
            ...
            if (req.url.startsWith('/api/user/') && req.method === 'GET') {
                ...
                if (!user) {
                    ...
                    return res.end(JSON.stringify({ message: '사용자를 찾을 수 없습니다.' }));
                }
                res.writeHead(200, { 'Content-Type': 'application/json' });
                return res.end(JSON.stringify({ id: userId, profile: user.profile, followers: user.followers }));
            }
            ...
            // 5. 회원 탈퇴
            if (req.url.startsWith('/api/user/') && req.method === 'DELETE' && !req.url.includes('/follower/')) {
                const userId = req.url.split('/').pop();
            ...
            if (req.url === '/api/user/follower/add' && req.method === 'POST') {
                ...
                if (me && userDb.has(targetId)) {
                    ...
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    return res.end(JSON.stringify({ message: '추가 성공', followers: me.followers }));
                }
            }
            if (req.url === '/api/user/follower/remove' && req.method === 'DELETE') {
                ...
                res.writeHead(200, { 'Content-Type': 'application/json' });
                return res.end(JSON.stringify({ message: '삭제 성공', followers: me ? me.followers : [] }));
            }
