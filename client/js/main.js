...
function updateSecondaryPanel(panelName) {
  const secondarySidebarContent = document.getElementById(
...
function renderSecondaryPanelHome() {
  ...
  const totalCalorie = todayMeals.reduce((sum, meal) => {
    return (
      sum +
      (meal.음식 || []).reduce((s, f) => s + (parseFloat(f.에너지) || 0), 0)
    );
  }, 0);
...
function renderSecondaryPanelDiet() {
  ...
  // 클릭 이벤트 추가
  document
    .querySelectorAll("#secondary-sidebar-content .secondary-item")
    .forEach((item) => {
...
function updateHomeStats() {
  ...
  const totalProtein = todayMeals.reduce((sum, meal) => {
    return (
      sum +
      (meal.음식 || []).reduce((s, f) => s + (parseFloat(f.단백질) || 0), 0)
    );
  }, 0);
...
function setupEventListeners() {
  ...
  if (document.getElementById("addMealForm")) {
    document
      .getElementById("addMealForm")
      .addEventListener("submit", handleAddMeal);
  }
  if (document.getElementById("foodSearch")) {
    document
      .getElementById("foodSearch")
      .addEventListener("input", handleFoodSearch);
  }
...
function applyFilters() {
  ...
  const filtered = dietData.filter((meal) => {
    ...
    const typeMatch =
      !currentFilters.mealType || meal.식사구분 === currentFilters.mealType;
...
function renderMealList(meals) {
  ...
  if (meals.length === 0) {
    container.innerHTML =
      '<div style="text-align: center; padding: 40px; color: #999;">조건에 맞는 식단이 없습니다.</div>';
    return;
  }
...
function calculateTotals(foods) {
  return {
    에너지: foods.reduce(
      (sum, food) => sum + (parseFloat(food.에너지) || 0),
      0,
    ),
    탄수화물: foods.reduce(
      (sum, food) => sum + (parseFloat(food.탄수화물) || 0),
      0,
    ),
    단백질: foods.reduce(
      (sum, food) => sum + (parseFloat(food.단백질) || 0),
      0,
    ),
